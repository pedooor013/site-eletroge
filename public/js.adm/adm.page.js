...
function checkWorkServices(services) {
    ...
    document
        .querySelectorAll('input[name="work-service"]')
...
document
    .querySelectorAll('input[name="work-service"]:checked')
...
if (workProgress < 0 || workProgress > 100) {
    alert("O progresso deve ser entre 0 e 100!");
    return;
}
const idService = [];
document
    .querySelectorAll('input[name="work-service"]:checked')
    .forEach((checked) => idService.push(Number(checked.id)));
