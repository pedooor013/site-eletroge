...
async function createRelationshipsWorkService(obraId, arrServicesId){
    try{
        console.log("🔧 Inserindo serviços para obra", obraId);
        console.log("Serviços:", arrServicesId);
        
        for(let countArrPosition = 0; countArrPosition < arrServicesId.length; countArrPosition++){
            console.log(`Inserindo serviço ${arrServicesId[countArrPosition]}`);
            await pool.query(`
                INSERT INTO obra_servico(obra_id, servico_id)
                VALUES($1, $2)
                `, [obraId, arrServicesId[countArrPosition]]); 
        }
        console.log("✅ Todos os serviços inseridos!");
    }catch(err){
        console.error("❌❌❌ ERRO REAL em createRelationshipsWorkService:", err);
        console.error("❌ Mensagem:", err.message);
        console.error("❌ Code:", err.code);
        console.error("❌ Detail:", err.detail);
        console.error("❌ Constraint:", err.constraint);
        console.error("❌ Table:", err.table);
        throw err; // ← JOGA O ERRO ORIGINAL, NÃO CRIA UM NOVO!
    }
}
    
async function createRelationshipsWorkImage(obraId, arrImage){
    try{
        console.log("🖼️ Inserindo imagens para obra", obraId);
        console.log("Imagens:", arrImage);
        ...
        console.log("✅ Todas as imagens inseridas!");
    }catch(err){
        console.error("❌❌❌ ERRO REAL em createRelationshipsWorkImage:", err);
        console.error("❌ Mensagem:", err.message);
        console.error("❌ Code:", err.code);
        console.error("❌ Detail:", err.detail);
        console.error("❌ Constraint:", err.constraint);
